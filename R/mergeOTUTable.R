...
## Merge OTUs with same assignments according to specific rank or best assignment (default)
mergeOTUsByTaxa = function(otu_table, taxon_table, rank="bestTx") {
  
  if (sum(names(otu_table) != row.names(taxon_table))>0){
    print("Error: OTU table does not correspond to taxon table")
    return()
  }
  
  tax_w_rank = taxon_table[!is.na(taxon_table[[rank]]),]
  otus_w_rank = otu_table[,!is.na(taxon_table[[rank]])]
  otus.t = as.data.frame(t(otus_w_rank))
  
  return(as.data.frame(t(mergeOTUTable(otus.t, tax_w_rank, by=rank))))
}
