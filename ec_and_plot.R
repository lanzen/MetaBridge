...
##
## Version history (started 2024)
## Added RPM for pesticide index and new synonym for PI piBI
## 2024-05-08 Added AQI2 and WQI2 (100-AQI or WQI)


SYNONYMS = data.frame(row.names = c("piEutro","pi_physchem", "PI5yAvg","PI10yAvg","PI.LT","PI","PI5y_sed","PAH_PI","THC_PI","Redox5yAvg","Redox10yAvg",
                                    "OM5yAvg","OM10yAvg","AMBIGroup","piMetals","piHC","piOM","piOM5yAvg", "altPI",
                                    "piOC","piBI","tsiSD","tsiChl","tsiND","tsiNO3","tsiNO2","tsiNH3","tsiPO4","tsiDO","Total.PRM",
                                    "AMBI_MOD", "ISI_MOD", "NQI1_MOD", "NSI_MOD"),
                      bi = c(rep("pi",9),"Redox","Redox","OM","OM","AMBI",rep("pi",7),
                             rep("TSI",8), "PRM", "AMBI", "ISI", "NQ1", "NSI"))
...
ECO_GROUPS = data.frame(row.names=c("AMBI","microgAMBI", "pi","Redox","OM","NSI",
                                    "NQI1","ISI","IntRatio6y","logC",
                                    "Nitrate","TSI","NSIneg","PRM", "AQI2",
                                    "WQI2", "nemerow", "ShannonNeg", "NQIneg","ISIneg"),
...
getBIGroupFromValue = function(value,bi="AMBI") {
  ...
  if (!bi %in% row.names(ECO_GROUPS)) {
    if (bi %in% row.names(SYNONYMS)) {
      ...
      write(paste0("Unknown PI (",bi,"). Breaking.\n"),stderr())
      return(NA)
    }
  }
  ...
  i=1
  #print(paste(i,bi,ECO_GROUPS[bi,i+1]))
  # print(bi)
  # print(value)
  # print(i)
  # print(ECO_GROUPS[bi,i+1])
  
  while (i < 6 & value > ECO_GROUPS[bi,i+1]) i=i+1
  
  if (bi=="NSI" | bi=="ISI" | bi=="NQI1") i = 6-i
...
getECFromPeak = function(value, bi="AMBI"){
  if (!bi %in% row.names(ECO_GROUPS)) {
    if (bi %in% row.names(SYNONYMS)) {
      ...
      write(paste0("Unknown BI (",bi,"). Breaking.\n"),stderr())
      return(NA)
    }
  }
  ...
  if (bi=="NSI" | bi=="ISI" | bi == "NQI1") ec = 6-ec
  return (ec)
}
...
getWeight= function(group, bi="AMBI"){
  ...
  if (bi %in% c("NSI","ISI","NQI1")) group = 6-group
...
plot_ml <- function(data, metadata, xIndex, yIndex, title = NULL, aggreg = NULL, pdf = F, taxo_group = NULL,
  ...
  comp <- metadata
  if(!is.null(aggreg)) {
    #if(length(aggreg)>1) comp$col_plot = as.numeric(droplevels(metadata[,aggreg[-1]]))
    if(length(aggreg)>1) comp$col_plot = as.numeric((metadata[,aggreg[-1]]))
    else comp$col_plot = as.numeric(metadata[,aggreg])
  }
  ...
  mod <- lm (pred.ag ~ train.ag) # 0+train.ag -> force thruogh origin
  ...
  plot(pred.ag ~ train.ag, xlim=getTotalRange(xIndex), ylim=getTotalRange(yIndex), 
       pch =pchs, col= col,  xaxs="i",yaxs="i",cex=1, main = paste(title), 
       xlab = xlab, ylab = ylab)
  ...
  if (yIndex=="NSI"  | yIndex=="ISI" | yIndex=="NQI1") groupCols = rev(groupCols)
  ...
  if (yIndex == "Redox" | yIndex == "Redox5yAvg" | yIndex == "Redox10yAvg") {
    ...
    if (xIndex == "NSIneg") xpos = -1
    else xpos = ECO_GROUPS[xI,6]
    text(xpos,-28, pos=2,paste("R²=", round(summary(mod)$adj.r.squared, 3), sig, sep=""))
    text(xpos,-29.5, pos=2,paste("Kappa=", round(kap$value, 3), sigk, sep=""))

    
  } else if (yIndex=="ShannonNeg") {
    ...
    if (xIndex == "ShannonNeg") xpos = -.2
    else xpos = ECO_GROUPS[xI,6]
      
    text(xpos,-5.5, pos=2,paste("R²=", round(summary(mod)$adj.r.squared, 3), sig, sep=""))
    text(xpos,-5.7, pos=2,paste("Kappa=", round(kap$value, 3), sigk, sep=""))
  
    
  } else if (yIndex=="ISIneg") {
    ...
    if (xIndex == "ISIneg") xpos = -.4
    else xpos = ECO_GROUPS[xI,6]
      
    text(xpos,-11, pos=2,paste("R²=", round(summary(mod)$adj.r.squared, 3), sig, sep=""))
    text(xpos,-11.5, pos=2,paste("Kappa=", round(kap$value, 3), sigk, sep=""))
    
    
  } else if (yIndex=="NQIneg") {
    ...
    if (xIndex == "NQIneg") xpos = -.15
    else xpos = 6
    
    text(xpos,-.9, pos=2,paste("R²=", round(summary(mod)$adj.r.squared, 3), sig, sep=""))
    text(xpos,-.95, pos=2,paste("Kappa=", round(kap$value, 3), sigk, sep=""))
  ...
  legend("topleft", as.vector(unique(farm_nam)), col= unique(col), pch=unique(pchs), box.lty=0, inset=c(0,0.11))
